...
export function parseSettings(): Settings {
    ...
    // Validación de opciones de espacio de color
    if ($('#optColorSpaceRGB').prop('checked')) {
        ...
        log('Por favor, selecciona un espacio de color válido.');
    }

    // Validación de eliminación de facetas
    settings.removeFacetsFromLargeToSmall = $('#optFacetRemovalLargestToSmallest').prop('checked');
    ...
    if (isNaN(settings.randomSeed) || settings.randomSeed < 0) {
        log('La semilla aleatoria debe ser un número positivo.');
        settings.randomSeed = 0; // Valor por defecto
    }
